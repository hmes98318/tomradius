    ...
    <div class="data-container">
        <div class="data-box">
            <div class="data-form">
                ...
                <el-input v-model="curData.mac_address" size="large" placeholder="輸入數值" />

                <label class="data-name">電腦名稱:</label>
                <el-input v-model="curData.computer_name" size="large" placeholder="輸入數值" />

                <label class="data-name">員工名稱:</label>
                <el-input v-model="curData.employee_name" size="large" placeholder="輸入數值" />

                <label class="data-name">其他描述: (選填)</label>
                <el-input v-model="curData.description" size="large" placeholder="輸入數值 (選填)" />
            </div>
            ...
            <div class='btn-container'>
                ...
                <el-button type="primary" class="el-btn" size="large" @click="putData()">修改</el-button>
            </div>
        </div>
    </div>
</template>
...
const emits = defineEmits(['SUCESS_PUT']);      // 添加事件傳回去給父組件
...
const getData = async (id: number) => {
    try {
        ...
        if (result.data.loadType === LoadType.SUCCEED) {
            ...
            errorMessage.value = '網頁參數設定，請聯絡管理員';
            return;
        }
        ...
        throw new Error("An unknown error occurred while put data: " + JSON.stringify(result.data));
    } catch (error) {
...
const putData = async () => {
    ...
    if (!curData.value.computer_name || curData.value.computer_name === '') {
        errorMessage.value = '請輸入有效電腦名稱';
        return;
    }
    if (!curData.value.employee_name || curData.value.employee_name === '') {
    ...
    try {
        ...
        const result: AxiosResponse<ResultData, any> = await axios.put(url, data);
        ...
        if (result.data.loadType === LoadType.SUCCEED) {
            ...
            errorMessage.value = '已存在該資料，請重新填寫';
            return;
        }
        else if (result.data.loadType === LoadType.DATA_NOT_FOUND) {
